...
public:
    ...
    bool intersect(const Ray& ray) override;//这个函数没写
    bool intersect(const Ray& ray, float& tnear,
                   uint32_t& index) const override;//这个函数没写
    ...
    Vector3f evalDiffuseColor(const Vector2f&) const override;//不知道
    Bounds3 getBounds() override;//算包围盒
